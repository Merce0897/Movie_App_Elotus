...
export default function MovieCard({ movie }: { movie: MovieCardProps }) {
  const imageServer = import.meta.env.VITE_IMAGE_URL;

  const { id, title, poster_path, releaseDate, overview, popularity } = movie;
  console.log("imageSrc", `${imageServer}/${poster_path}`);
  return (
